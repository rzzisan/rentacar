...
$stats = [
    'total_vehicles'     => (int)($q("SELECT COUNT(*) c FROM vehicles WHERE id IN $in", $vids, $t)['c'] ?? 0),
    'available_vehicles' => (int)($q("SELECT COUNT(*) c FROM vehicles WHERE id IN $in AND status='available'", $vids, $t)['c'] ?? 0),
    'active_rentals'     => (int)($q("SELECT COUNT(*) c FROM rentals WHERE vehicle_id IN $in AND rental_status='active'", $vids, $t)['c'] ?? 0),
    'pending_rentals'    => (int)($q("SELECT COUNT(*) c FROM rentals WHERE vehicle_id IN $in AND rental_status='pending'", $vids, $t)['c'] ?? 0),
    'monthly_revenue'    => (float)($q(
        "SELECT COALESCE(SUM(CASE WHEN agreed_amount > 0 THEN agreed_amount ELSE COALESCE(total_amount,0) END),0) r
         FROM rentals WHERE vehicle_id IN $in AND rental_status != 'cancelled'
    ...
    'total_dues'         => (float)($q(
        "SELECT COALESCE(SUM(s.remaining_amount),0) d FROM settlements s
         ...
         WHERE r.vehicle_id IN $in AND s.payment_status != 'paid'",
        $vids, $t
    )['d'] ?? 0),
    'today_trips'        => (int)($q(
        "SELECT COUNT(*) c FROM rentals WHERE vehicle_id IN $in
