...
**কাজের তালিকা:**
- [x] `android/app/build.gradle.kts`-এ প্রকৃত version bump প্রথা চালু করা (versionCode/versionName এখন পর্যন্ত সবসময় `1`/`1.0.0` স্থির ছিল — কখনো বাড়েনি) → প্রথমে ম্যানুয়ালি `2`/`1.1.0`, তারপর `3`/`1.1.1`-এ বাম্প করা হয়েছিল, কিন্তু **ম্যানুয়াল বাম্প নিজেই একটা fragile single point of failure** ধরা পড়ে (versionCode সারা প্রজেক্টের ইতিহাসে একবারও বাড়েনি এই আবিষ্কার হওয়ার আগ পর্যন্ত)। তাই হার্ডেন করা হয়েছে: `versionCode` এখন `gitCommitCount()` ফাংশন দিয়ে **স্বয়ংক্রিয়ভাবে** `git rev-list --count HEAD` থেকে বসে (কমিট সংখ্যা কখনো কমে না, তাই monotonic guarantee ফ্রি) — মানুষের মনে রাখার উপর আর নির্ভর করে না। `versionName` (মানুষের পড়ার জন্য, semver স্টাইল) এখনো ম্যানুয়াল, কিন্তু update-check লজিক এটার উপর নির্ভর করে না। **⚠️ ordering সতর্কতা:** versionCode build-সময়ে HEAD থেকে বসে, তাই সবসময় আগে সব কোড পরিবর্তন commit করো, তারপর build করো — নাহলে build-এ বেক হওয়া versionCode আর পরে deploy-সময় গণনা করা version.json-এর সংখ্যা mismatch হতে পারে।
...
- [x] `gen-index.sh` স্ক্রিপ্ট হার্ডেন করা: এখন `build.gradle.kts` থেকে versionCode আর গ্রেপ করে না (versionCode এখন literal সংখ্যা না, ফাংশন কল — গ্রেপ চুপচাপ ব্যর্থ হতো) — বরং build.gradle.kts-এর `gitCommitCount()`-এর ঠিক একই কমান্ড (`git rev-list --count HEAD`) স্বাধীনভাবে চালিয়ে হিসাব করে, যাতে APK-তে বেক হওয়া versionCode আর version.json-এর সংখ্যা কম্পিউটেশন-পদ্ধতির অমিলের কারণে কখনো ভিন্ন না হয়। এছাড়া নতুন safety check যোগ হয়েছে: নতুন গণনা করা version_code পুরনো `version.json`-এর মানের চেয়ে বড় না হলে জোরালো সতর্কতা (`❌❌❌`) দেখায় (যেমন commit না করেই build করা হলে) — deploy আটকায় না, কিন্তু মানুষকে সতর্ক করে। (ঐচ্ছিক আর্গুমেন্ট: changelog, force_update)
