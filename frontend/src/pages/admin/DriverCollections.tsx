...
const DriverCollections: React.FC = () => {
  ...
  const loadDrivers = async () => {
    setLoading(true);
    try {
      const res = await api.get<{ drivers: DriverDues[]; summary: typeof summary }>('/admin/drivers/dues.php');
  ...
  const loadDetail = async (driverId: number) => {
    ...
    setDetailError('');
    try {
      const res = await api.get<DriverDuesDetail>(`/admin/drivers/dues.php?id=${driverId}`);
      if (res.success && res.data) {
        setDetail(res.data);
      } else {
  ...
  const handleCollect = async () => {
    ...
    try {
      const res = await api.post<{ collected_amount: number; allocations: CollectionAllocation[] }>(
        `/admin/drivers/collect.php?id=${detail.driver.id}`,
        {
